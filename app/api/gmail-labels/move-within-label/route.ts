...
export async function POST(request: Request) {
  try {
    ...
    if (!body.sourceLabelId || !body.targetLabelId || !body.senders) {
      return Response.json({ error: "Missing sourceLabelId, targetLabelId, or senders" }, { status: 400 });
    }
    let moved = 0;
    for (const sender of body.senders) {
      const msgs = await gmail.users.messages.list({
        userId: "me",
        labelIds: [body.sourceLabelId],
        q: `from:${sender}`,
        maxResults: 500,
      });
      ...
      if (ids.length) {
        await gmail.users.messages.batchModify({
          ...
          requestBody: {
            ids,
            addLabelIds: [body.targetLabelId],
            removeLabelIds: [body.sourceLabelId],
          },
        });
        moved += ids.length;
      }
